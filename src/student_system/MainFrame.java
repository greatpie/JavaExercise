...
//                student[i] = str.split(" ");
                ...
                studentArr[i] = new Student(studentStr);
            }
    ...
    public int Search(String FileName,int num,String information)
    {
        ...
        String str = null;
        Student stutemp[] = new Student[100];
...
//                    flag = 1;
//                }
              stutemp[i] = new Student(str.split(" "));
                if(stutemp[i].getByNum(num).equals(information))
                {
                    studentArr[count] =stutemp[i];
                    count ++;
                    flag = 1;
                }
            }
...
//                temp[i] = str.split(" ");
                stutemp[i] = new Student(str.split(" "));
                if(stutemp[i].getByNum(num).equals(information[num]))
...
//                                total += Double.parseDouble(temp[i][j]);
                                total += Double.parseDouble(stutemp[i].getByNum(j));
                            }
                            if(j == 0)
                                tempStr = stutemp[i].getByNum(j);
                            else
                                tempStr=tempStr+" "+stutemp[i].getByNum(j);
                            continue;
                        }
                        if(j>=6)
                        {
                        ...
                        stutemp[i].setByNum(j,information[j]);
                        if(j == 0)
                            tempStr=stutemp[i].getByNum(j);
                        else
                            tempStr=tempStr+" "+stutemp[i].getByNum(j);
                    ...
                    stutemp[i].total = String.valueOf(total);stutemp[i].average = String.valueOf((int)(total/4));
                }
                studentArr[i] = stutemp[i];
...
//        }
        try
        {
            ...
            for(int i =0;(str = br.readLine())!=null;i++)
            {
               studentArr[i]= new Student(str.split(" "));
                count++;
            }
            for (int j = 0;j < count;j++)
            {
                ...
                {
                    Student temp;
                    d1 = Double.parseDouble(studentArr[i].getByNum(num));
                    d2 = Double.parseDouble(studentArr[i+1].getByNum(num));
...
//        }
        try
        {
            ...
            for(int i =0;(str = br.readLine())!=null;i++)
            {
                count ++;
                stutemp[i] = new Student(str.split(" "));
                total +=Double.parseDouble(stutemp[i].getByNum(num));
            }
        }
        catch (IOException e)
        {
...
//       private String[][] data = student;
        private String[][] data = toStringArray();

        public String[][] toStringArray(){
            String[][] data = new String[studentArr.length][12];
            for (int i= 0;i<studentArr.length;i++){
                try {
                    data[i] = studentArr[i].toStringArray();
                }catch (NullPointerException npec){
                    npec.printStackTrace();
                }
            }
            return  data;
        }
